...
use App\Service\Orders\Commands\StartApprovalFlow;
...
class Command extends Query
{
    public function save(object $dto)
    {
        return match (true) {
            ...
            $dto instanceof StartApprovalFlow => $this->startApprovalFlow($dto),
     ...
     * 承認フロー開始
     */
    private function startApprovalFlow(StartApprovalFlow $dto): void
    {
        DB::transaction(function () use ($dto) {
            // ロック取得
            $this->lockForUpdate($dto->id, $dto->updatedAt);
